...
module appletini_yarz_top (
    ...
    // Only A2CTRL.RESET is populated on the production board. It carries
    // both the boot-time hold and every virtual-card RESET request through
    // the dedicated open-collector transistor. A2FPGA.RESET is observation
    // only. The other planned assertion pins are physically unconnected;
    // IRQ is driven low/high-Z through the bidirectional A2FPGA.IRQ lane.
