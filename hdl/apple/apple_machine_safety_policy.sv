...
module apple_machine_safety_policy (
    ...
    input  logic       bootstrap_identity_valid,
    input  logic       bootstrap_identity_legacy,
    input  logic       bootstrap_identity_iigs,
    ...
    always_comb begin
        machine_identity_fault = machine_id_fault ||
                                 iigs_slot_policy_fault ||
            (bootstrap_identity_valid && (locked_machine_id != 4'd0) &&
             ((bootstrap_identity_iigs && (locked_machine_id != 4'd4)) ||
              (bootstrap_identity_legacy && (locked_machine_id == 4'd4))));
        machine_identity_reported = (locked_machine_id != 4'd0);
        /* A legacy ID grants physical rights only with trusted bootstrap
         * provenance. This low-only classifier deliberately never produces
         * such a verdict, so a PS/ONEe-spoofed ID1-3 stays fail closed. */
