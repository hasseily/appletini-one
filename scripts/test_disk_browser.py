...
Only filesystem I/O, preview work and mount actions are stubbed. The tests use
...
def main() -> int:
    ...
    names = (
        ...
        "config_menu_has_disk2_ext", "config_menu_has_png_ext",
    ...
    tests = textwrap.dedent(r'''
        ...
        static void test_smartport_initial_and_disabled_rows(void)
        {
            ...
            add_file("z.po", 1U, 0U); add_file("Readme.txt", 4096U, 0U);
            ...
            CHECK(g_browser_entries[find_entry(&menu, "Readme.txt")].type == CONFIG_BROWSER_ENTRY_FILTERED,
            ...
            menu.browser_selected = find_entry(&menu, "Readme.txt");
        ...
        static void test_fallback_directory_and_scroll(void)
        {
            ...
            char name[40];
            reset_fixture(&menu, CONFIG_BROWSER_TARGET_DISK2_D1);
            add_file("notes.txt", 100U, 0U); add_file("Child", 0U, AM_DIR);
            config_menu_browser_set_dir(&menu, "0:/disks");
        ...
        static void test_capacity_and_empty_movement(void)
        {
            ...
            for (unsigned i = 0; i < CONFIG_BROWSER_MAX_ENTRIES + 4U; ++i) {
                snprintf(name, sizeof(name), "filtered%03u.txt", i); add_file(name, 1U, 0U);
            }
        ...
        int main(void)
        {
            ...
            puts("DISK BROWSER NATIVE PASS: 6 behavior groups");
            return 0;
        }
    ''')
