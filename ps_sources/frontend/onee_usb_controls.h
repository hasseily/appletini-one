...
#define ONEE_USB_ROUTE_HOLD_TOGGLE (1U << 1)
#define ONEE_USB_ROUTE_DELAY_ACTION (1U << 2)
...
{
    return (usage == HID_KBD_USAGE_PRINTSCN ||
...
static inline uint8_t onee_usb_fixed_route(
    ...
    usb_hid_menu_source_t open_close_source)
{
    if (source == open_close_source) {
        return (uint8_t)(ONEE_USB_ROUTE_HOLD_TOGGLE |
            ((action != USB_HID_MENU_ACTION_NONE) ?
                ONEE_USB_ROUTE_DELAY_ACTION : 0U));
    }
    if (reserved != 0U && action != USB_HID_MENU_ACTION_NONE) {
        return ONEE_USB_ROUTE_PUSH_NOW;
    }
