...
/*****************************************************************************/
/**
* IRQ-guarded EP0 (control) send.
*
* XUsbPs_EpBufferSend (XUsbPs_EpQueueRequest) primes the endpoint via a
* non-atomic dQH-overlay / ATDTW (USBCMD) / ENDPTPRIME read-modify-write
* sequence with no internal locking. These EP0 replies run from the USB ISR,
* but the controller's poll-context EP0 re-prime path touches the same
* ENDPTPRIME register, so a preempting USB interrupt can corrupt an in-flight
* prime and leave the dTD active-but-never-serviced. Mask IRQ around every EP0
* send to keep priming atomic. The restore only re-enables IRQ when it was
* enabled on entry, so this is safe from ISR context (IRQ already masked).
******************************************************************************/
static int XUsbPs_Ep0GuardedSend(XUsbPs *InstancePtr, const u8 *BufferPtr,
				 u32 BufferLen)
{
	u32 Cpsr = mfcpsr();
	int Status;
	...
	Status = XUsbPs_EpBufferSend(InstancePtr, 0U, (u8 *)BufferPtr, BufferLen);
...
void __attribute__((weak)) usb_storage_debug_note_ch9_error(
	const XUsbPs_SetupData *SetupData)
{
	(void)SetupData;
}

void __attribute__((weak)) usb_storage_debug_note_ep0_stall(void)
{
}

void __attribute__((weak)) usb_storage_debug_note_clear_feature_halt(u8 EpNum)
{
	(void)EpNum;
}

void __attribute__((weak)) usb_storage_debug_note_set_address(u16 Address)
{
	(void)Address;
}

void __attribute__((weak)) usb_storage_debug_note_set_configuration(u16 Config)
{
	(void)Config;
}

void __attribute__((weak)) usb_storage_debug_note_descriptor(u8 DescType,
							    u16 RequestLen,
							    u32 ReplyLen)
{
	(void)DescType;
...
void __attribute__((weak)) usb_storage_debug_note_ep_send_failure(u8 EpNum,
								  u32 BufferLen,
								  int Status)
{
	(void)EpNum;
...
#endif
			/* Active USB0 personality gets first refusal (the FT60x
			 * emulation lives entirely on vendor requests). */
			if (usb0_personality_vendor_req(InstancePtr, SetupData,
							&Status)) {
				break;
			}
...
#pragma data_alignment = 32
	...
	switch (SetupData->bRequest) {
		...
		case XUSBPS_REQ_GET_STATUS:
			...
			XUsbPs_Ep0GuardedSend(InstancePtr, Reply, SetupData->wLength);
			break;
		...
		case XUSBPS_REQ_SET_ADDRESS:
			 ...
			 */
			usb_storage_debug_note_set_address(SetupData->wValue);
			 ...
			 * zero length packet.
			 */
			XUsbPs_Ep0GuardedSend(InstancePtr, NULL, 0);
			break;
		...
		case XUSBPS_REQ_GET_INTERFACE:
			...
			/* Ack the host */
			XUsbPs_Ep0GuardedSend(InstancePtr, &Response, 1);
		...
		case XUSBPS_REQ_GET_DESCRIPTOR:
			...
			switch ((SetupData->wValue >> 8) & 0xff) {
				...
				case XUSBPS_TYPE_DEVICE_QUALIFIER:
					...
					Status = XUsbPs_Ep0GuardedSend(InstancePtr,
								     Reply, ReplyLen);
					usb_storage_debug_note_descriptor(
						(u8)((SetupData->wValue >> 8) & 0xff),
						SetupData->wLength, (u32)ReplyLen);
					if (XST_SUCCESS != Status) {
						usb_storage_debug_note_ep_send_failure(0,
										       (u32)ReplyLen,
										       Status);
				...
				case 0x0F: /* BOS descriptor (bcdUSB >= 2.01) */

					/* Only the SDD vendor personality carries a BOS
					 * (for the MS OS 2.0 WinUSB auto-binding); the
					 * storage personality replies 0 and the empty
					 * reply falls through to a stall below. */
					ReplyLen = usb0_personality_bos_desc(
							   Reply, XUSBPS_REQ_REPLY_LEN);
					if (ReplyLen == 0U) {
		...
		case XUSBPS_REQ_SET_CONFIGURATION:
			...
			usb_storage_debug_note_set_configuration(
		...
		case XUSBPS_REQ_GET_CONFIGURATION:
				...
				XUsbPs_Ep0GuardedSend(InstancePtr,
			...
			switch (SetupData->bmRequestType & XUSBPS_STATUS_MASK) {
				case XUSBPS_STATUS_ENDPOINT:
					if (SetupData->wValue == XUSBPS_ENDPOINT_HALT) {
						...
						usb_storage_debug_note_clear_feature_halt(
							(u8)EpNum);
						...
						if (EpNum & 0x80) {	/* In ep */
							...
							/* USB 2.0 9.4.5: ClearFeature(ENDPOINT_HALT)
							 * must reinitialize the data toggle to DATA0.
							 * Clearing the stall (TXS) does NOT do this on
							 * ChipIdea; we must pulse the TX toggle-reset
							 * bit. Otherwise the device keeps its old toggle
							 * while the host (which always resets to DATA0
							 * after ClearFeature) is on DATA0 -> every IN
							 * packet is dropped as a retransmission and the
							 * endpoint stalls silently: dTD stays active,
							 * EPRDY stuck, no halt/error bit. This is the
							 * BOT error-recovery path Windows runs after a
							 * bulk hiccup. */
		...
		case XUSBPS_REQ_SET_INTERFACE:
			...
			Error = XUsbPs_Ep0GuardedSend(InstancePtr, NULL, 0);
			if (Error) {
#ifdef CH9_DEBUG
