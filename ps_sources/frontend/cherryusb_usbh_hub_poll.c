...
static cherryusb_host_debug_t g_host_debug[CONFIG_USBHOST_MAX_BUS];
...
static void usbh_hub_events(struct usbh_hub *hub)
{
    ...
    int ret;
    size_t flags;
    cherryusb_host_debug_t *debug = NULL;
    ...
    if (hub->bus != NULL && hub->bus->busid < CONFIG_USBHOST_MAX_BUS) {
        debug = &g_host_debug[hub->bus->busid];
        debug->event_calls++;
        if (hub->is_roothub) {
            debug->root_event_calls++;
        }
        debug->last_hub_index = hub->index;
        debug->last_hub_is_root = hub->is_roothub ? 1U : 0U;
    }
    ...
    if (debug != NULL) {
        debug->last_portchange_index = portchange_index;
    }
    ...
    for (uint8_t port = 0; port < hub->nports; port++) {
        ...
        if (debug != NULL) {
            debug->port_event_count++;
            debug->last_hub_index = hub->index;
            debug->last_hub_is_root = hub->is_roothub ? 1U : 0U;
            debug->last_port = port + 1U;
        }

        /* Read hub port status */
        ret = usbh_hub_get_portstatus(hub, port + 1, &port_status);
        if (debug != NULL) {
            debug->last_get_status_ret = ret;
        }
        if (ret < 0) {
        ...
        if (debug != NULL) {
            debug->last_port_status = portstatus;
            debug->last_port_change = portchange;
        }

        USB_LOG_DBG("port %u, status:0x%03x, change:0x%02x\r\n", port + 1, portstatus, portchange);

        child = &hub->child[port];
        if ((portstatus & HUB_PORT_STATUS_CONNECTION) != 0U &&
            child->connected == false &&
            (portchange & HUB_PORT_STATUS_C_CONNECTION) == 0U) {
            USB_LOG_INFO("Port %u already connected; synthesizing connect change\r\n", port + 1);
            if (debug != NULL) {
                debug->synth_connect_count++;
            }
            port_status.wPortChange |= HUB_PORT_STATUS_C_CONNECTION;
        ...
        while (portchange) {
            if (portchange & mask) {
                ...
                if (debug != NULL) {
                    debug->last_clear_feature_ret = ret;
                }
                if (ret < 0) {
        ...
        if (portchange & HUB_PORT_STATUS_C_CONNECTION) {
            ...
            for (uint32_t debouncetime = 0; debouncetime < HUB_DEBOUNCE_TIMEOUT; debouncetime += HUB_DEBOUNCE_STEP) {
                ...
                portchange = port_status.wPortChange;
                if (debug != NULL) {
                    debug->last_debounce_status = portstatus;
                    debug->last_debounce_change = portchange;
                    debug->last_debounce_stable_ms = debouncestable;
                }
            ...
            if (portstatus & HUB_PORT_STATUS_CONNECTION) {
                ...
                ret = usbh_hub_get_portstatus(hub, port + 1, &port_status);
                if (debug != NULL) {
                    debug->last_post_reset_status_ret = ret;
                }
                if (ret < 0) {
                    USB_LOG_ERR("Failed to read port %u status, errorcode: %d\r\n", port + 1, ret);
                    continue;
                ...
                portchange = port_status.wPortChange;
                if (debug != NULL) {
                    debug->last_reset_status = portstatus;
                    debug->last_reset_change = portchange;
                }
                ...
                if (!(portstatus & HUB_PORT_STATUS_RESET) && (portstatus & HUB_PORT_STATUS_ENABLE)) {
                    ...
                    */
                    ...
                    if (debug != NULL) {
                        debug->enumerate_count++;
                        debug->last_enumerate_ret = ret;
                    }
                    if (ret < 0) {
...
int usbh_hub_initialize(struct usbh_bus *bus)
{
    ...
    if (usb_hc_init(bus) < 0) {
        USB_LOG_ERR("Failed to init host controller\r\n");
...
void cherryusb_host_poll(uint8_t busid)
{
    ...
    uint32_t portsc;
    uint32_t now_ms;
    static uint32_t s_last_root_probe_ms[CONFIG_USBHOST_MAX_BUS];
    ...
    g_host_debug[busid].last_root_portsc = portsc;
    now_ms = cherryusb_baremetal_ms();
    if (hub->connected &&
        hub->nports > 0U &&
        hub->child[0].connected == false &&
        (portsc & 0x00000001U) != 0U) {
        g_host_debug[busid].root_probe_candidates++;
        if ((now_ms - s_last_root_probe_ms[busid]) >= 1000U) {
            s_last_root_probe_ms[busid] = now_ms;
            g_host_debug[busid].root_probe_attempts++;
            g_host_debug[busid].root_probe_synths++;
            hub->int_buffer[0] |= 0x02U;
            usbh_hub_events(hub);
        }
    }
    ...
    for (uint8_t i = 0U; i < 8U; ++i) {
        if (usb_osal_mq_recv(bus->hub_mq, &hub_value, 0U) < 0) {
            break;
        }

        hub = (struct usbh_hub *)hub_value;
        if (hub != NULL) {
            g_host_debug[busid].mq_events++;
            usbh_hub_events(hub);
        }
    }
}
...
void cherryusb_host_debug_note_control(const struct usbh_hubport *hport,
                                       ...
                                       int ret)
{
    cherryusb_host_debug_t *debug;

    if (hport == NULL || hport->bus == NULL ||
        hport->bus->busid >= CONFIG_USBHOST_MAX_BUS) {
        return;
    }

    debug = &g_host_debug[hport->bus->busid];
    debug->control_count++;
    if (ret < 0) {
        debug->control_fail_count++;
    }
    debug->last_control_ret = ret;
    debug->last_control_dev_addr = hport->dev_addr;
    debug->last_control_speed = hport->speed;
    debug->last_control_ep0_mps = hport->ep0.wMaxPacketSize;
    if (hport->parent != NULL) {
        debug->last_hub_index = hport->parent->index;
    }
    debug->last_port = hport->port;

    if (setup != NULL) {
        debug->last_control_request_type = setup->bmRequestType;
        debug->last_control_request = setup->bRequest;
        debug->last_control_value = setup->wValue;
        debug->last_control_index = setup->wIndex;
        debug->last_control_length = setup->wLength;
    }
}
                                        ...
                                        uint16_t remaining)
{
    ...
    debug = &g_host_debug[busid];
    debug->ehci_error_count++;
    debug->last_ehci_qtd_index = qtd_index;
    debug->last_ehci_qtd_token = token;
    debug->last_ehci_qtd_pid = (uint8_t)((token >> 8) & 0x03U);
    debug->last_ehci_qtd_length = length;
    debug->last_ehci_qtd_remaining = remaining;
    debug->last_ehci_qtd_actual = (length >= remaining) ?
...
void cherryusb_host_debug_note_ehci_zlp_babble_ignored(uint8_t busid)
{
    ...
    g_host_debug[busid].ehci_zlp_babble_ignored++;
}
                                          ...
                                          uint8_t stage)
{
    ...
    g_host_debug[hport->bus->busid].last_enum_stage = stage;
}
                                             ...
                                             uint8_t started)
{
    ...
    if (hport == NULL || hport->bus == NULL ||
        hport->bus->busid >= CONFIG_USBHOST_MAX_BUS ||
        intf >= CONFIG_USBHOST_MAX_INTERFACES) {
        return;
    }
    ...
    debug->class_connect_count++;
    if (ret < 0) {
        debug->class_connect_fail_count++;
    }
    debug->last_class_ret = ret;
    debug->last_class_driver = driver_name;
    debug->last_class_intf = intf;
    debug->last_class_class = intf_desc->bInterfaceClass;
    debug->last_class_subclass = intf_desc->bInterfaceSubClass;
    debug->last_class_protocol = intf_desc->bInterfaceProtocol;
    debug->last_class_dev_addr = hport->dev_addr;
    debug->last_class_speed = hport->speed;
    debug->last_class_started = started;
}
