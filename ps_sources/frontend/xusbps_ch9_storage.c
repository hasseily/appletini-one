...
void __attribute__((weak)) usb_storage_debug_note_ep_prime(u8 EpNum,
							   u8 Direction,
							   int Status)
{
	(void)EpNum;
...
*
******************************************************************************/
u32 XUsbPs_Ch9SetupDevDescReply_Storage(u8 *BufPtr, u32 BufLen)
{
...
*
******************************************************************************/
u32 XUsbPs_Ch9SetupCfgDescReply_Storage(u8 *BufPtr, u32 BufLen)
{
	USB_CONFIG config = {
		/* Std Config */
		{
...
*
******************************************************************************/
u32 XUsbPs_Ch9SetupStrDescReply_Storage(u8 *BufPtr, u32 BufLen, u8 Index)
{
	u32 i;
...
*
******************************************************************************/
void XUsbPs_SetConfiguration_Storage(XUsbPs *InstancePtr, int ConfigIdx)
{
	int Status;
	...
	/* USB bus reset clears on-chip endpoint state but not the dTD ring or dQH
	 * in DRAM. A primed, undrained IN descriptor can therefore block the first
	 * transfer after configuration. Flush and reinitialize both EP1 directions
	 * while SET_CONFIGURATION guarantees that no transfer is in flight. EpFlush
	 * handles one direction per call. */
	XUsbPs_EpFlush(InstancePtr, 1, XUSBPS_EP_DIRECTION_IN);
	XUsbPs_EpFlush(InstancePtr, 1, XUSBPS_EP_DIRECTION_OUT);
	(void)XUsbPs_ReconfigureEp(InstancePtr, &InstancePtr->DeviceConfig, 1,
				   XUSBPS_EP_DIRECTION_IN, 0);
	(void)XUsbPs_ReconfigureEp(InstancePtr, &InstancePtr->DeviceConfig, 1,
				   XUSBPS_EP_DIRECTION_OUT, 0);

	/* Prime the OUT endpoint. */
	Status = XUsbPs_EpPrime(InstancePtr, 1, XUSBPS_EP_DIRECTION_OUT);
	usb_storage_debug_note_ep_prime(1, XUSBPS_EP_DIRECTION_OUT, Status);
}
